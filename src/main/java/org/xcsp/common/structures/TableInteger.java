...
public class TableInteger extends Table {
	...
	public TableInteger intersectionWith(TableInteger other) {
		...
		return new TableInteger().add(setOfSupports.stream()); // .toArray(new int[setOfSupports.size()][]);
	}

	public TableInteger addColumnWithValue(int position, int value) {
		int[][] m = this.toArray();
		Utilities.control(0 <= position && position <= m[0].length, "bad value of column position");
		return new TableInteger().add(IntStream.range(0, m.length)
